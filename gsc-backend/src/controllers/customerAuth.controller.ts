...
export async function register(req, res) {
  try {
    ...
    // No tokens yet — the account can't log in until the email is verified.
...
export async function verifyEmail(req, res) {
  try {
    ...
    let payload;
    try {
      payload = verifyJwt(token, CUSTOMER_JWT_SECRET);
    } catch {
      return res
        .status(401)
...
export async function refreshToken(req, res) {
  try {
    ...
    let payload;
    try {
      payload = verifyJwt(token, CUSTOMER_REFRESH_SECRET);
    } catch {
      return res
        .status(401)
